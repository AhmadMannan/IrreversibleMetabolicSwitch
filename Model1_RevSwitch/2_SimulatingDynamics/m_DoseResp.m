...
for i = 1:length(OA)
    ...
    R = linspace(1e-10,Rmax*1.1,300);
    
    
    % --- Generating master nullcline (MN) -------------------------------
    
    [~,MN] = m_Obj_NullclinesAndSS(R,params,OA(i),arch);
%     figure(7)
%     if i == 1
%         clf
%     end
...
%     end
    ...
    for j = 1:length(pos_chg(:,1))
        lb = rng_R(j,1);
        ub = rng_R(j,2);
        [R_ss_t,fval] = m_Bisection(@(rss)m_Obj_NullclinesAndSS(rss,params,OA(i),arch),lb,ub);
        if abs(fval) <= 1e-4
            % R_ss = [R_ss; OA(i) R_ss_t];
            rss_t = [rss_t; R_ss_t];
        end
    ...
    for j = 1:length(rss_t)
        ...
        [~,~,SS_t] = m_Obj_NullclinesAndSS(rss_t(j),params,OA(i),arch);
        
        % Checking stability of steady state (eigenvalues of Jacobian):
        J_t = m_Jacobian(params,arch,SS_t,OA(i));
        ev_t = eig(J_t);
        if max(real(ev_t)) <= 0
            stable = 1;
        else
